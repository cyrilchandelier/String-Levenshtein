...
class ViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        hintLabel.preferredMaxLayoutWidth = view.frame.width - 2 * 15;
    }
    ...
    fileprivate func search() {
        ...
        results = filterableArray.filtered(using: NSPredicate(format: "SELF CONTAINS[cd] %@", searchString)) as! [String]
        tableView.reloadData()
        
        // If there is no results, try to find close words
        self.tableView.tableHeaderView = nil
        if results.count == 0 && searchString.count >= 2 {
            let closeWords = self.closeWords(from: searchString, words: dictionary)
            if closeWords.count > 0 {
                ...
                for word in closeWords {
                    ...
                    if i != 0 && i == closeWords.count - 1 {
                        hint.append(NSAttributedString(string: " or "))
                    } else if i > 0 {
                        hint.append(NSAttributedString(string: ", "))
                    }
                    
                    // Append word in a different style
                    hint.append(NSAttributedString(string: word.uppercased(), attributes: [
...
extension ViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        cell.textLabel?.text = results[indexPath.row].capitalized
    }
